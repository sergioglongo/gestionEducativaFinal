...
import { COURSE_STUDENTS, SET_PAY_TO_CART, DELETE_PAY_FROM_CART ,GO_CHECKOUT, DONT_GO_CHECKOUT} from "../Components/Pay/actionsPay";
...
export default function reducer(state = initialState, actions) {
  ...
  switch (actions.type) {
    ...
    case GO_CHECKOUT:
      return { ...state, goCheckOut: "cart" }
    case DONT_GO_CHECKOUT:
      return { ...state, goCheckOut: "" }
    case SET_PAY_TO_CART:
      return { ...state, pays: [...state.pays, actions.payload] }
    case DELETE_PAY_FROM_CART:
      return { ...state, pays: state.pays.filter(pay => pay.idPayStudents !== actions.payload) }
    case LOADING:
