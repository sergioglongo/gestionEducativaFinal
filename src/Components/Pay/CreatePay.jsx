...
export default function CreatePay() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!input.students[0] || !input.subject || !input.amount || !input.expirationDate || !input.detail) return swal("Comprobar! ", "todos los campos debe de esta completados correctamente", "warning")
    createPay(input)
  ...
  return (
    <Box sx={{ mt: 10 }} >
      <Card variant="outlined" style={{ margin: "0 auto", pading: "20px 5px" }} sx={{ maxWidth: 500 }}>
        ...
        <Grid xs={12} sm={6} sx={{ m: 1 }} item>
          <TextField id="outlined-textarea" error={error.detail} label={error.messageDetail} placeholder="Ingrese detalle" multiline size="small" value={input.detail} name="detail" onChange={handleChange} />
        </Grid>
        <Grid xs={12} sm={6} sx={{ m: 1 }} item>
          <Autocomplete
            size="small"
