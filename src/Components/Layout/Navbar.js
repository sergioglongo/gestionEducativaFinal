...
export default function PrimarySearchAppBar({ mobileOpen, handleDrawerToggle }) {
  ...
  function handledPayButton(e){
    if(pays.length===0) return swal("Debe seleccionar pagos", { buttons: false, timer: 2000})
    navigate("/payments")
    dispatch(goCheckout())
  }
      ...
      <MenuItem>
        {typeUser === "Tutor"
          ? <IconButton
            ...
            color="inherit"
            onClick={handledPayButton}
          >
            <Badge badgeContent={pays?pays?.length:"0"} color="error">
              <MonetizationOnIcon />
            </Badge>
          </IconButton>
          : ""}
          ...
          <Box sx={{ display: { xs: 'none', md: 'flex' } }}>
            ...
            <MenuItem>
              {typeUser === "Tutor"
                ...
                >
                  <Badge badgeContent={pays?pays?.length:"0"} color="error">
                    <MonetizationOnIcon sx={{ width: 35, height: 35 }}/>
                  </Badge>
                </IconButton>
                : ""}
