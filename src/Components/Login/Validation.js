...
export default function Validation(name, value) {
  ...
  if (name === "amount") {
    if (value.toString().length >= 8) return error = { type: "amount", amount: "Longitud maxima 8 caracteres", status: "error" }
  ...
  if (name === "detail") {
    ...
    if (value === "") {
      ...
      return error = { type: "detail", detail: "Solo debe tener hasta 160 caracteres", status: "stop" }
    } if (value.length <= 5) {
